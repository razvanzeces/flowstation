...
impl CcBsSubentity {
    ...
    pub(in crate::cmce::subentities::cc_bs) fn fsm_on_u_connect(
        ...
        // Called MS connect-ack:
        //   - Duplex P2P: Granted, permission=false — both MS may speak simultaneously,
        //     audio is cross-routed UL↔DL between the two timeslots (see UMAC rx_tmd_prim).
        //   - Simplex P2P (PTT): GrantedToOtherUser — calling holds the floor right now,
        //     called listens on DL and may request the floor by sending U-TX-DEMAND.
        //
        // NOTE: ETSI EN 300 392-2 §14.8.43: transmission_request_permission is encoded as
        //   0 = ALLOWED to request transmission
        //   1 = NOT ALLOWED to request transmission
        // The Rust bool name is misleading: `false` here means "permission granted to request".
        // Sending `true` (= 1 = not allowed) prevents the called MS from ever pressing PTT.
        let (called_grant, called_perm) = if simplex_duplex {
            (TransmissionGrant::Granted, false)
        } else {
            (TransmissionGrant::GrantedToOtherUser, false)
        };
        let d_connect_ack = DConnectAcknowledge {
            call_identifier: call_id,
            call_time_out: CallTimeout::T5m.into_raw() as u8,
            transmission_grant: called_grant.into_raw() as u8,
            transmission_request_permission: called_perm,
            notification_indicator: None,
            facility: None,
            proprietary: None,
        };
