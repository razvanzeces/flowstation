...
impl CcBsSubentity {
    ...
    pub(in crate::cmce::subentities::cc_bs) fn fsm_on_u_tx_ceased(
        ...
        if let Some(call) = self.individual_calls.get(&call_id).cloned() {
            // For simplex PTT individual calls: MS released PTT, notify both parties so the
            // floor is fully relinquished and either side may request it next with U-TX-DEMAND.
            if !call.is_active() {
            ...
            tracing::info!("U-TX CEASED (individual) call_id={} from ISSI {} -> notifying both parties (sender + peer ISSI {})", call_id, sender.ssi, peer_addr.ssi);

            // ETSI 14.8.43: permission=false means "allowed to request transmission".
            // Both sides must be allowed to PTT after the floor is released, otherwise the
            // call deadlocks (no one can ever press PTT again, neither side can speak).
            let build_d_tx_ceased = || DTxCeased {
            ...
            // 1) D-TX-CEASED back to the SENDER, so its radio resets the local floor state
            //    and re-enables the local PTT input. Without this, the radio still believes
            //    it owns the floor and will refuse to send another U-TX-DEMAND.
            tracing::info!("-> D-TX CEASED (individual simplex, FACCH) call_id={} to sender ISSI {}", call_id, sender.ssi);
            let mut ceased_self_sdu = BitBuffer::new_autoexpand(30);
            build_d_tx_ceased()
                .to_bitbuf(&mut ceased_self_sdu)
                .expect("Failed to serialize DTxCeased (self)");
            ceased_self_sdu.seek(0);
            let ceased_self_msg = Self::build_sapmsg_stealing(ceased_self_sdu, sender, sender_ts, Some(sender_usage));
            queue.push_back(ceased_self_msg);

            // 2) D-TX-CEASED to the PEER, signaling the floor is now free.
            tracing::info!("-> D-TX CEASED (individual simplex, FACCH) call_id={} to peer ISSI {}", call_id, peer_addr.ssi);
            let mut ceased_peer_sdu = BitBuffer::new_autoexpand(30);
            build_d_tx_ceased()
                .to_bitbuf(&mut ceased_peer_sdu)
                .expect("Failed to serialize DTxCeased (peer)");
            ceased_peer_sdu.seek(0);
            let ceased_peer_msg = Self::build_sapmsg_stealing(ceased_peer_sdu, peer_addr, peer_ts, Some(peer_usage));
            queue.push_back(ceased_peer_msg);
            return;
        }
    ...
    pub(in crate::cmce::subentities::cc_bs) fn fsm_on_u_tx_demand(
        ...
        if let Some(call) = self.individual_calls.get(&call_id).cloned() {
            ...
            // D-TX-GRANTED to requester (Granted)
            ...
            let dtg_req_msg = Self::build_sapmsg_stealing(dtg_req_sdu, requesting_party, req_ts, Some(req_usage));
            queue.push_back(dtg_req_msg);

            // D-TX-GRANTED to peer (GrantedToOtherUser).
            // ETSI 14.8.43: permission=false means "allowed to request transmission".
            ...
            dtg_peer_sdu.seek(0);
            let dtg_peer_msg = Self::build_sapmsg_stealing(dtg_peer_sdu, peer_addr, peer_ts, Some(peer_usage));
