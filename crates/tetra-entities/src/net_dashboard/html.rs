...
<style>
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  header {
    ...
    gap: 14px; height: 52px; flex-shrink: 0;
    position: relative;
  }
  header::before {
  ...
  .logo-icon {
    width: 28px; height: 28px; border: 2px solid var(--accent); border-radius: 3px;
    display: flex; align-items: center; justify-content: center;
    font-size: 13px; color: var(--accent); font-family: var(--mono); font-weight: 600;
    box-shadow: 0 0 10px rgba(0,200,150,0.25), inset 0 0 8px rgba(0,200,150,0.08); flex-shrink: 0;
  }
  .logo-name { font-family: var(--mono); font-size: 13px; font-weight: 600; color: var(--accent); letter-spacing: 0.04em; white-space: nowrap; }
  .logo-sub  { font-family: var(--mono); font-size: 9px; color: var(--text2); letter-spacing: 0.1em; text-transform: uppercase; white-space: nowrap; }
  .hdiv { width: 1px; height: 26px; background: var(--border2); flex-shrink: 0; }
  ...
  .bts-ip-value { font-family: var(--mono); font-size: 12px; font-weight: 600; color: var(--text2); letter-spacing: 0.04em; }
  ...
  .status-label { font-family: var(--mono); font-size: 11px; color: var(--text2); letter-spacing: 0.05em; }
  /* Lang + theme controls */
  ...
  .lang-btn.active { background: rgba(0,200,150,0.15); color: var(--accent); }
  .theme-btn.active { background: rgba(0,200,150,0.15); color: var(--accent); }
  .theme-sep { width: 1px; height: 16px; background: var(--border2); }
  nav { display: flex; gap: 1px; margin-left: auto; flex-shrink: 0; }
  .tab {
  ...
  /* Main */
  main { padding: 16px 20px; max-width: 1400px; margin: 0 auto; width: 100%; flex: 1; }
  .page { display: none; }
  ...
  .stat-val  { font-family: var(--mono); font-size: 26px; font-weight: 600; color: var(--text); line-height: 1; }
  ...
  th { text-align:left; font-family:var(--mono); font-size:8px; font-weight:600; text-transform:uppercase; letter-spacing:0.12em; color:var(--text3); padding:5px 8px 7px; border-bottom:1px solid var(--border); }
  td { padding:8px; border-bottom:1px solid var(--border); color:var(--text); font-size:13px; }
  ...
  .rssi-track{ flex:1; height:3px; background:var(--bg4); border-radius:2px; overflow:hidden; min-width:40px; }
  ...
  /* Buttons */
  .btn { background:var(--bg3); border:1px solid var(--border2); color:var(--text2); padding:5px 10px; border-radius:var(--r); cursor:pointer; font-family:var(--mono); font-size:10px; font-weight:500; letter-spacing:0.05em; transition:all 0.15s; white-space:nowrap; }
  ...
  /* Footer */
  footer { flex-shrink:0; background:var(--bg2); border-top:1px solid var(--border); padding:0 20px; height:34px; display:flex; align-items:center; gap:12px; font-family:var(--mono); font-size:10px; color:var(--text3); letter-spacing:0.05em; }
  ...
  /* Responsive — 7" and smaller */
  @media (max-width:640px) {
    header { height:auto; min-height:52px; padding:8px 12px; flex-wrap:wrap; gap:8px; }
    .logo-sub    { display:none; }
    .hdiv        { display:none; }
    .bts-ip-block{ display:none; }
    nav { order:10; width:100%; justify-content:center; border-top:1px solid var(--border); margin:4px -12px 0; padding:4px 0 2px; }
    .tab { padding:5px 10px; font-size:9px; }
    main { padding:10px 10px; }
    .stats-row { grid-template-columns:1fr 1fr; gap:8px; }
    .stat-val { font-size:22px; }
    td,th { padding:6px 5px; font-size:12px; }
    footer { font-size:9px; gap:8px; height:30px; padding:0 10px; }
...
<div class="page active" id="page-stations">
  ...
  <div class="card">
    ...
    <table>
      ...
      <tbody id="ms-tbody">
        <tr><td colspan="5"><div class="empty-state"><div class="empty-icon">📡</div><div class="empty-text" data-i18n="no_terminals">No terminals registered</div></div></td></tr>
      </tbody>
    </table>
  </div>
</div>
...
<footer>
  ...
  <span class="footer-right">TETRA FlowStation v0.0.8</span>
</footer>
...
function setLang(l, btn) {
  ...
  if (btn) btn.classList.add('active');
    ...
    document.querySelectorAll('.lang-btn').forEach(b => {
      if (b.textContent.toLowerCase() === l) b.classList.add('active');
    });
  }
  applyLang();
}
...
function setTheme(theme, btn) {
  ...
  if (btn) btn.classList.add('active');
    ...
    const map = {'dark':0,'white':1,'blue':2};
...
let ws=null,state={ms:{},calls:{}},sdsDest=0;
