...
/// A duplex echo-999 call must NOT inherit the Infinite p2p duplex call timeout. The echo peer is
/// virtual, so without a finite timer a caller that simply drops (no U-DISCONNECT/U-RELEASE) would
/// leak its circuit and timeslot forever — there is no MS link supervision to reclaim it. The cap is
/// T5m, which check_call_timeout_expiry uses to release the slot. We assert the on-air D-CONNECT,
/// which carries the same value the internal call is reaped on.
#[test]
fn test_local_echo_999_duplex_call_is_capped_to_finite_timeout() {
    debug::setup_logging_verbose();
    ...
    // Duplex selection = true — the case that previously yielded an Infinite timeout.
    test.submit_message(build_individual_u_setup_msg_with_mode(calling_issi, LOCAL_ECHO_ISSI, true));
    test.run_stack(Some(1));
    let msgs = test.dump_sinks();
    ...
    assert_eq!(
        d_connect.call_time_out,
        CallTimeout::T5m,
        "duplex echo call must be capped at a finite timeout so a dropped caller's slot is reclaimed"
    );
}
