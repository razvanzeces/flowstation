...
fn default_bind_addr() -> String {
    // Loopback by default — same posture as the dashboard. Listening on all interfaces is opt-in:
    // an operator who runs the MeshCom node on another host sets this to a LAN address (or 0.0.0.0).
    "127.0.0.1".to_string()
}
...
pub fn apply_meshcom_patch(src: CfgMeshcomDto) -> Result<CfgMeshcom, String> {
    ...
    Ok(CfgMeshcom {
        enabled: src.enabled,
        bind_addr: non_empty_or(src.bind_addr, "127.0.0.1"),
